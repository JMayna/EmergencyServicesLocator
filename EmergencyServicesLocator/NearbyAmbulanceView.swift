...
struct NearbyAmbulanceView: View {
    
    @StateObject private var locationManager = LocationManager()
    @State private var ambulanceServices: [MKMapItem] = []
    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 30) {
                ...
                Text("Nearest Ambulance Services")
                    .font(.largeTitle.bold())
                    .padding(.top, 20)
                    .foregroundColor(Color(red: 10/255, green: 57/255, blue: 102/255))
                
                if isLoading {
                    ProgressView("Searching for nearby ambulance services…")
                        .padding()
                }
                ...
                else if !ambulanceServices.isEmpty {
                    VStack(spacing: 20) {
                        ForEach(ambulanceServices.prefix(3), id: \.self) { service in
                            ambulanceCard(service: service)
                                .padding(.horizontal)
                        }
                    }
                }
        ...
        .onReceive(locationManager.$lastLocation) { location in
            ...
            if ambulanceServices.isEmpty && errorMessage == nil {
    ...
    // MARK: - Ambulance Card
    func ambulanceCard(service: MKMapItem) -> some View {
        
        VStack(alignment: .leading, spacing: 12) {
            
            Text(service.name ?? "Unknown Ambulance Service")
                .font(.title3.bold())
                .foregroundColor(Color(red: 25/255, green: 40/255, blue: 70/255))
            
            if let address = service.placemark.postalAddress {
            ...
            if let phone = service.phoneNumber {
                Text("Phone: \(phone)")
                    .foregroundColor(.gray)
            }
            
            if let distance = distanceText(to: service) {
    ...
    // MARK: - Search Ambulance Locations
    func fetchAmbulanceServices() {
        
        guard let location = locationManager.lastLocation else { return }
        
        let request = MKLocalSearch.Request()
        
        // Try common search terms for best results
        request.naturalLanguageQuery = "ambulance service"
        ...
        search.start { response, error in
            ...
            guard let items = response?.mapItems, !items.isEmpty else {
                errorMessage = "No ambulance services found nearby."
                isLoading = false
                return
            }
            ...
            ambulanceServices = Array(sorted.prefix(3))
